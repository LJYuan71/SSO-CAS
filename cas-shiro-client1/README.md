...
转载至：https://github.com/coder-huang/sso-shiro-cas
## 系统模块说明
1.  cas：  单点登录模块，这里直接拿的是cas的项目改了点样式而已
2.  doc：   文档目录，里面有数据库生成语句，采用的是MySQL5.0，数据库名为db_test  
3.  cas-shiro-clent1：   应用1
4.  cas-shiro-clent2：   应用2

  其中shiro1跟shiro2都是采用spring + springMVC + mybatis 框架，使用maven做项目管理
...
<!-- 设置密码的加密方式，这里使用的是MD5加密 -->
  ...
  <!-- 设置数据源 -->
	 <bean id="dataSource" class="org.springframework.jdbc.datasource.DriverManagerDataSource">
		  ...
		  <property name="url" value="jdbc:mysql://127.0.0.1:3306/cas-test?useUnicode=true&amp;characterEncoding=utf8"></property>
		  <property name="username" value="root"></property>
		  <property name="password" value="root"></property>  
  </bean>
```
  其中JdbcAuthenticationHandler这个类是自定义身份认证
...
``` properties
shiro.loginUrl=http://127.0.0.1:8080/cas/login?service=http://127.0.0.1:8081/shiro1/shiro-cas

shiro.logoutUrl=http://127.0.0.1:8080/cas/logout?service=http://127.0.0.1:8081/shiro1/shiro-cas

shiro.cas.serverUrlPrefix=http://127.0.0.1:8080/cas

shiro.cas.service=http://127.0.0.1:8081/shiro1/shiro-cas
...
3.在resources/conf/shiro.xml 文件中
``` xml
<!-- Shiro Filter -->
...
最后我们还需要在webapp/WEB-INF/web.xml 文件中配置相关的过滤器拦截全部请求
``` xml
<filter>
...
> shiro1：http://127.0.0.1:8081/

> shiro2：http://127.0.0.1:8082/

3.访问系统
> 输入  http://127.0.0.1:8081/(需要授权的资源)，进入cas验证

> 输入管理员admin或经理mg8888或用户yh8888，密码均为admin@2015，验证成功后将会重定向到http://127.0.0.1:8081/node1//users/loginSuccess ，也就是node1系统的主页，里面的节点2代表的是node2系统的主页，你会发现我们不需要登录到node2系统就能访问其中的系统了
